...
## Amendment — M1/M2 findings (2026-08-26; Tasks 13/14 injection + capture superseded)

1. **Input injection is message-driven (no key-state global).** M1 verified the binary has NO GetKeyState/GetAsyncKeyState and NO WM_KEYUP: input is edge-triggered WM_KEYDOWN through `wproc_main` (0x405800) only. Task 13 Step 1's `ski_key_pressed`/GetKeyState design is superseded:
   - **Rebuild (T13):** a `#if SKI_HARNESS` hook at game-tick start (inside `ski_tick`) consumes the per-tick input word and, for each set bit, directly calls `ski_wproc_main(g_c6c8, 0x100, SKI_VK_i, 0)` (direct wproc call verified deterministic in T10). Non-harness builds are untouched (the message loop remains the only input path).
   - **Original (T14):** the injected `.stub` at the 0x401000 tick entry does the same per set bit: call `wproc_main(0x405800)` with `(c6c8, 0x100, vk, 0)` using the original's own globals (c6c8 @ 0x40c6c8, c67c active gate checked first, as the real message path does).
2. **Input format: 2 bytes/tick (u16 LE), 12-bit layout.** 11 scenario keys > 8 bits, so Task 13's one-byte-per-tick format is superseded: `ski_in.bin` = 2 bytes per tick; bit 0=left, 1=right, 2=up, 3=down, 4=crouch, 5=facing1, 6=facing3, 7=facing7, 8=facing9, 9=restart(F2), 10=pause(F3), 11=advance(Enter). A set bit at word t (0-based) = exactly ONE WM_KEYDOWN(vk) injected at the start of game tick t+1 (no repeat, no up). Scenario `"down": false` events are no-ops (no keyup path exists). Generated by `harness/gen_input.py` (also emits `src/ski_keys.h`); VK symbols from `harness/seed.json` (verified 2026-08-25/26 keymap).
3. **Frame capture at the tick boundary, not EndPaint.** Task 13 Step 2's EndPaint dump is superseded (paints are not tick-aligned; the scene is drawn by the tick, not by paint). Symmetric contract: **frame_N = main client DC after game tick N's body, before word[N] injection.**
   - Rebuild: dump at tick N END (`ski_tick` exit): `frame_%06d_main.ppm` (P6 top-down 24bpp).
   - Original: the stub has one hook point (tick entry) — dump frame_{N−1} at tick N START, i.e. frame_N is captured at tick N+1 start (after N's body, before word[N] is consumed).
   - A scenario with T input words runs T+1 ticks; `harness/run_scenario.sh` waits for `frame_<T>_main.ppm`. diff.py compares frames 1..T (`--min-tick 1`).
4. **Harness scripts are already committed (20e9dfd)** — Task 13 Steps 3–4 are done, except the C-side hook: `harness/gen_input.py` (all 8 T8 scenarios pass; bit layout + seed.json VK symbols), `harness/diff.py` (tick-aligned PPM diff; scene mask 620,0,760,60 per b6facf5; `--tol/--shift/--viz/--report`; synthetic 1-px/mask/shift tests pass), `harness/run_scenario.sh` (gen → launch wine as the script's child from OUT_DIR so ski_in.bin resolves from CWD → stall/death detection → teardown), `harness/cap_x11.sh` (manual wall-clock X11 capture `cap_%07d_main.png` — visual only, NOT the diff path; verified against the live original, 760×734 geometry window filter).
5. **Operational constraints (Wine/Xvfb on this host, verified 2026-08-26):**
   - A wine instance backgrounded from a short-lived shell is reaped when that command's process group ends — long-lived observation needs a supervisor-managed process (pty) or a parent script that outlives the launch (run_scenario.sh is safe: wine is its child).
   - Original boot to first window under this Xvfb takes ~20 s; scenario budgets in run_scenario.sh (60 ms/tick + 60 s) cover it.
   - A crashed instance leaves a modal "Program Error" + "Wine Debugger" window on the display, which blocks relaunch via the single-instance guard — cleanup: kill wine + `DISPLAY=:99 xdotool windowkill <id>` (search `--name "Program Error"` / `"Wine Debugger"`).
   - The wine game child's `/proc/<pid>/cmdline` is the PE path itself (no `wine` prefix); a bare `pkill -f ski.exe` from a script whose argv contains the exe path kills the script — use `pkill -f '(^|/)[^ ]*ski\.exe$'` (matches only cmdlines ending in the PE path).
   - Xvfb :99 (1024×768×24) is the only display; do not start a second Xvfb on :99.
